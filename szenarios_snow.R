...
snow.szenario.1.3 <- function() {
  ...
  CPU.hours.fluctuating <- as.list(ifelse((snowset.largest.cust.hourly$totalCpuTime - 344) < 0, 0, (snowset.largest.cust.hourly$totalCpuTime - 344)))
...
# Cover all with base except from highest spike
snow.szenario.1.4 <- function() {
  
  CPU.hours.required <- 472
  CPU.hours.fluctuating <- as.list(ifelse((snowset.largest.cust.hourly$totalCpuTime - 472) < 0, 0, (snowset.largest.cust.hourly$totalCpuTime - 472)))
...
# Cover all with base except from highest spike, but can only commit for 1 year
snow.szenario.1.5 <- function() {
  
  CPU.hours.required <- 472
  CPU.hours.fluctuating <- as.list(ifelse((snowset.largest.cust.hourly$totalCpuTime - 472) < 0, 0, (snowset.largest.cust.hourly$totalCpuTime - 472)))
  
  snow.szenario.1.5.avg <<- find.cheapest.instance.final(CPU.hours.required, CPU.hours.fluctuating, 2, duration = "1")
  total.price.base.avg <- snow.szenario.1.5.avg[1, 'Total_Costs'] * 24 
  total.price.fluct.avg <- sum(snow.szenario.1.5.avg[2:25, 'Total_Costs']) 
  snow.szenario.1.5.total.daily.avg <<- total.price.base.avg + total.price.fluct.avg
  
  snow.szenario.1.5.min <<- find.cheapest.instance.final.min(CPU.hours.required, CPU.hours.fluctuating, 2, duration = "1")
  total.price.base.min <- snow.szenario.1.5.min[1, 'Total_Costs'] * 24 
  total.price.fluct.min <- sum(snow.szenario.1.5.min[2:25, 'Total_Costs']) 
  snow.szenario.1.5.total.daily.min <<- total.price.base.min + total.price.fluct.min
}
snow.szenario.1.5()
...
snow.szenario.2.3 <- function() {
  ...
  CPU.hours.fluctuating <- as.list(ifelse((snowset.large.cust.hourly$totalCpuTime - 123) < 0, 0, (snowset.large.cust.hourly$totalCpuTime - 123)))
